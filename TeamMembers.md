# Team Members

## Purpose

Short internal notes about the project team. This file is a personal reference used to:

- remember who is who;
- keep context for communication;
- support AI-assisted message review;
- speed up project onboarding and daily work;

## Project Context

- Company: Coca - cola;
- Sub Project: Promo Plus;
- Project location: Sofia, Bulgaria;
- My status: External contractor;
- My location: Bulgaria;
- Language note: I am not a native speaker;

## How To Use This File

- Keep entries short;
- Prefer facts over opinions;
- Mark uncertain information explicitly;
- Use this file as context for writing messages and understanding team structure;
- Use `Nickname` values as stable identifiers in ticket metadata (`owners`);

## Team Operating Model

- [verified, 2026-04-15, source: team lead confirmation] There is no dedicated DevOps team member in Promo Plus;
- [verified, 2026-04-15, source: team lead confirmation] DevOps and infrastructure work are shared responsibilities across the team;
- [verified] For infra tasks, assign a clear owner per ticket/work item, even when implementation is collaborative;

## Team Members

### Tanya Tanevska

- Full Name: Tanya Tanevska;
- Nickname: tanyaTanevska;
- Role: Director, IT Marketing;
- Project familiarity: Core project member;
- Location: Sofia;
- Contribution type: not contributing source code;
- Language: first language bulgarian;

### Maria Dobreva

- Full Name: Maria Dobreva;
- Nickname: mariaDobreva;
- Role: Team leader of Promo plus;
- Project familiarity: Core project member;
- Location: Sofia;
- Contribution type: Software developer;
- Language: first language bulgarian;
- Notes: Team lead for Promo Plus;

### Irena Simeonova

- Full Name: Irena Simeonova;
- Nickname: irenaSimeonova;
- Role: Manager, Scrum Master of Promo plus;
- Project familiarity: Core project member;
- Location: Sofia;
- Contribution type: not contributing source code;
- Language: first language bulgarian;

### Vessela Velinova-Slancheva

- Full Name: Vessela Velinova-Slancheva;
- Nickname: vesselaVelinovaSlancheva;
- Role: Marketing Technologies Product Owner
- Project familiarity: Core project member;
- Location: Sofia;
- Contribution type: not contributing source code;
- Language: first language bulgarian;

### Marin Roglev

- Full Name: Marin Roglev;
- Nickname: marinRoglev;
- Role: Director, Solution Architect;
- Project familiarity: Core project member;
- Location: Sofia;
- Contribution type: Solution Architect;
- Language: first language bulgarian;
- Notes: Marin has several certificates from AWS;
